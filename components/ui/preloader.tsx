...
export default function Preloader({ onComplete }: PreloaderProps) {
  ...
  return (
    <motion.div
      ...
      className={`fixed inset-0 w-screen h-screen flex items-center justify-center z-[99999999999] transition-all duration-1000 ${
        fadeOut ? "bg-red-700/40 backdrop-blur-md" : "bg-red-700"
      }`}
    >
      {dimension.width > 0 && (
        <>
          ...
          <svg className="absolute top-0 w-full h-[calc(100%+300px)]">
            <motion.path
              ...
              initial="initial"
              animate={isExiting ? "exit" : "initial"}
              fill="#7f1d1d" // darker red curve fill
            />
          </svg>
        </>
      )}
